...
const ArbitratorManagement = () => {
  ...
  const filteredArbitrators = useMemo(() => {
    ...
    return arbitrators.filter(
      (arbitrator) =>
        ...
        arbitrator.qualification.toLowerCase().includes(lowercasedSearch) ||
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.imageFile) {
        try {
          const imageUrl = await uploadImageToImgBB(formData.imageFile);
      ...
      const response = await axiosSecure.post("/add-arbitrator", finalFormData);
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this arbitrator?")) {
      try {
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          <div className="bg-white rounded-xl p-4 shadow-lg border border-blue-100">
            ...
            <div className="text-gray-600 text-sm">
              {searchTerm ? "Filtered Arbitrators" : "Total Arbitrators"}
            </div>
          </div>
            ...
            <div className="text-2xl font-bold text-green-600">
              {
                filteredArbitrators.filter((a) =>
                  a.specialization.includes("arbitration trainer"),
                ).length
              }
            </div>
            <div className="text-gray-600 text-sm">Arbitration Trainers</div>
          </div>
          <div className="bg-white rounded-xl p-4 shadow-lg border border-purple-100">
            <div className="text-2xl font-bold text-purple-600">
              {new Set(filteredArbitrators.flatMap((a) => a.languages)).size}
            </div>
        ...
        {searchTerm && (
          <div className="mb-6 p-4 bg-blue-50 rounded-xl border border-blue-200">
            <p className="text-blue-700">
              ...
              {filteredArbitrators.length === 0 && " - No arbitrators found"}
            </p>
          </div>
        )}
        ...
        {filteredArbitrators.length === 0 ? (
          <div className="text-center py-12">
            ...
            <h3 className="text-xl font-semibold text-gray-600 mb-2">
              {searchTerm ? "No Arbitrators Found" : "No Arbitrators Found"}
            </h3>
